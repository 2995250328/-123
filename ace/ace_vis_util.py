...
def save_one_sc_map_in_txt2(image_B1HW, sc, gt_inv_pose, K, file):
    """
    save 1 sc map to txt file, later render it with meshlab
    ...
    pixel_grid = get_pixel_grid(8)  # Shape: 2x5000x5000

    B, C, H, W = sc.shape

    # scene coordinate to camera coordinates
    pred_scene_coords_B3HW = sc.float()
...
def save_one_sc_map_in_txt(image_B1HW, sc, gt_inv_pose, K, file):
    """
    save 1 sc map to txt file, later render it with meshlab
    ...
    scene = file[0].split('/')[-4]


    B, C, H, W = sc.shape
    # breakpoint()
    oor = 10 # math.inf #10 # set hard coded max/min range
    sc = torch.clip(sc, min=-oor, max=oor)


    ''' parse XYZ '''
    XYZ = sc[0].reshape(3,-1) # 3x4800

    '''parse normalize XYZ'''
    max_XYZ = XYZ.max(dim=1)[0][:,None] # [3,1]
    min_XYZ = XYZ.min(dim=1)[0][:,None] # [3,1]
    # normalize to 0-1
    RGB = (XYZ-min_XYZ)/(max_XYZ-min_XYZ) # [3,4800]

    ''' Save RGB image '''
    save_dir2 = f'tmp/{scene}/single_frame_sc_RGB'
    os.makedirs(save_dir2, exist_ok=True)
    _, fname = osp.split(file[0])
    save_img_name = fname[:-4] + '.png'
    RGB_img = RGB.reshape(B,C,H,W)

    save_imgname = osp.join(save_dir2, save_img_name)
    save_image(RGB_img, save_imgname)

    '''Save X Y Z R G B '''
    save_dir = f'tmp/{scene}/single_frame_sc_point_cloud'
    os.makedirs(save_dir, exist_ok=True)
    _, fname = osp.split(file[0])
    save_fname = fname[:-4]+'.txt'
    with open(osp.join(save_dir, save_fname), 'w') as out:
        for i in range(XYZ.shape[1]):
            line = f'{XYZ[0,i]}, {XYZ[1,i]}, {XYZ[2,i]}, {RGB[0,i]}, {RGB[1,i]}, {RGB[2,i]},\n'
            out.write(line)



def get_point_cloud_from_network(network, data_loader, filter_depth, with_transformer_head=False):
    """
    ...
    with torch.no_grad():
        ...
        for batch in data_loader:
            image = batch['image']
            gt_inv_pose = batch['pose_inv']
            K = batch['intrinsics']
            file = batch['rgb_files']
            ...
            with autocast():
                if with_transformer_head:
                    scene_coords = network.forward_ACE(image)
                else:
