...
class ACEVisualizer:
    ...
    def _render_frame_from_buffers(self):
        ...
        r = pyrender.OffscreenRenderer(self.render_width, self.render_height, point_size=self.point_size) # invalid deviceid error?
    ...
    def setup_reloc_visualisation(self, frame_count, network, data_loader, camera_z_offset, reloc_frame_skip, with_transformer_head=False):
        """
        ...
        @param with_transformer_head: If True, we are inferencing marepo network, if False, we are inferencing ACE
        """
        ...
        try:
            ...
            _logger.info(f"Extract fully trained map from network.")
            map_xyz, map_clr = vutil.get_point_cloud_from_network(network, data_loader, self.map_depth_filter, with_transformer_head=with_transformer_head)
